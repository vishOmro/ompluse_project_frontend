...
const GetSender = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess("Sender retrieved successfully!");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    console.log("Updated Sender:", resource);
  }, [resource]);
  ...
  return (
    <div className="px-4 py-3">
      ...
      <div className="search_create flex w-full justify-around mt-4">
        ...
        {/* <a href="/dlt-manager/create-dlts/create-sender"> */}
        <button className="px-8 py-2 bg-blue-600 text-white rounded-lg cursor-pointer">
          Create Sender
        </button>
        {/* </a> */}
            ...
            {resource.length > 0 ? (
              resource.map((item, index) => (
                <tr key={index} className="border-t border-t-[#dce0e5]">
                  ...
                  <td className="table-column-240 h-[72px] px-4 py-2 w-60 text- text-sm font-normal leading-normal">
                    <button
